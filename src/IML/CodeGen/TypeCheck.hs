...
data Context = Context { -- sammelt alle Variablen
    globals :: [GlobalImport], -- übner Ident Type check
    params :: [Param],
    progParams :: [ProgParam],
...
typeCheckCommands c l = typeCheckMain' True c l
    where typeCheckCommands' acc _  []                                                 = acc
          ...
          typeCheckCommands' acc co ((CallCommand ident exprLi [Ident])     :commands) = typeCheckCommands' (typeCheckProcedureCallParams co exprLi && scopeCheck co ident) co commands  -- Ist ein Aufruf für eine Prozedur
...
typeCheckExpr c l = typeCheckExpr' True c l
    where typeCheckExpr' acc _  []                                          = acc
          ...
          typeCheckExpr' acc co ((NameExpr ident isBool)            :exprs) = typeCheckExpr' (scopeCheck co ident)
...
getExprType (LiteralExpr (Literal ty))          =
